...
I built this to understand how cloud storage actually works —
...
- Restores all files from the cloud if data is lost

## Scripts
- backup.sh — creates a timestamped backup file and uploads it to GCS
- restore.sh — restores everything back from GCS to local when needed
